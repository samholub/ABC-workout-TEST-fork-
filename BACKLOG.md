...
| 6  | OPEN   |        | `sw.js` hardcodes `CACHE_NAME = 'abc-workout-v2'` with no tie to the app version, so a deploy to main serves the previous `index.html` from cache until the name is bumped by hand. |
