...
| 10 | OPEN   |        | `sw.js` answers a navigation from the cache and only revalidates in the background (`return cached \|\| networkFetch`, `sw.js:48`), so the first open after every deploy renders the previous build and the new one appears a launch later. |
...
| 11 | OPEN   |        | Row 9 writes the draft through when a set is completed, but the same handler still leaves un-completing a set or editing a logged weight/rep value to the 5s `setInterval`, so a crash within those 5s restores the set the user just corrected or cleared. |
